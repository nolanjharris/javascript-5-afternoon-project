...
/* Write a constructor function called User. This function should take in 4 parameters called 
name, age, email, and savedPosts in that order. Name and email will be strings, age will be a number 
and savedPosts will be an array of objects. These objects will each have 3 properties: id (a number), 
title (a string), and rating (a number between 1 and 5). These objects are the posts that the user will
...
//in one number parameter representing the post id. Use this id to find and remove the matching object 
//in the savedPosts array.

// Code here

User.prototype.removeSavedPost = function (num) {
  let index = this.savedPosts.findIndex(e => e.id === num);
...
// that will take in two number parameters. The first will be an id (a number) and the 
// second will be the new rating (a number). Use the id to find the matching object in 
// the savedPosts array. Once you find the matching object, update it's rating score with 
// the new rating parameter.
